...
const verify = (req,res,resolve,reject, rights) => async(err,user) => {
    ...
    if(rights.length){
        ...
        if(!permission.granted) {
            return reject(new ApiError(httpStatus.FORBIDDEN, "Sorry, you don't have the rights for this action."))
        }
...
const auth = (...rights) => async(req,res,next) =>{
    return new Promise((resolve,reject)=>{
        passport.authenticate('jwt', { session: false }, verify(req,res,resolve,reject,rights))(req,
        res.next)  // verify() is a cb func
